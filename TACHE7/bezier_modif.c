...
Bezier2 approx_bezier2(TTV_Point seq){
	Bezier2 b2;
	...
	if (n==2){
		...
		for (int i=1;i<n;i++){
			b2.c0=seq.tab[0];
			ti=i/nr;
			b2.c1.x=(seq.tab[i].x-pow(1-ti,2)*seq.tab[0].x-pow(ti,2)*seq.tab[seq.nb-1].x)/(2*ti*(1-ti));
			b2.c1.y=(seq.tab[i].y-pow(1-ti,2)*seq.tab[0].y-pow(ti,2)*seq.tab[seq.nb-1].y)/(2*ti*(1-ti));
			b2.c2=seq.tab[seq.nb-1];
